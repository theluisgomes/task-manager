...
- [x] Task comments / activity log
- [ ] Project timeline / Gantt view
- [x] Export KPI data to CSV (Excel export still pending — `xlsx` unused)
- [x] Dark mode toggle
- [ ] Mobile-responsive board view (partial — column stacking only)

## Modo Completo (cliente)
- [x] Vista estratégica + prioridade
- [x] Multi-assignee
- [x] Dashboard deadlines (overdue + view all)
- [x] Team employment contracts + HH cost
- [x] Finance contract P&L
- [x] Payment reminder e-mails
- [x] Página CRM
- Ver: `docs/Task-Manager/Relatorio-Modo-Completo.md`

## Maintenance & tooling
- [x] `pnpm run doctor` — diagnostic suite (env, DB, integrity, deps)
- [x] `pnpm run maintenance` — weekly routine with conservative auto-fix
- [x] `GET /health` — includes MySQL connectivity check
- [x] `db:generate` / `db:migrate` split (deploy uses migrate only)
- [x] CI database smoke test (migrate + doctor --smoke)
