...
### Custom domain
Cloud Run → service → **Manage custom domains** → add your domain → add the DNS
records Google gives you → then update the OAuth redirect URI and re-run `deploy`
(the `OAUTH_REDIRECT_BASE_URL` will need to point at the custom domain).
...
- Migrations run via `pnpm db:push` (`drizzle-kit generate && migrate`)
